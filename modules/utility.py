...
def compute_with_aa(data,thermalization=50):
    ...
    for i,(name,datas) in enumerate(data.items()):
        try:
            np.savetxt("../modules/error_temp/temp_file.txt",datas[thermalization:,:])
        except:
            np.savetxt("../modules/error_temp/temp_file.txt",datas[thermalization:])
...
def compute_with_aa_jackknife(data,column,bins,one_index=None,thermalization=1000):
    ...
    for i,(name,datas) in enumerate(data.items()):
        ...
        if one_index: 
            np.savetxt("../modules/error_temp/temp_file.txt",datas[thermalization:,-1])
        else:
            np.savetxt("../modules/error_temp/temp_file.txt",datas[thermalization:])
...
# def compute_with_fsh_jackknife(data,column,bins,system_size,range_and_acc,path="/home/haaaaron/SUN_twist_python_analysis/modules/fsh_temp/",thermalization_range=(1000,2000)):
#     fsh_list_lines = []
...
#         np.savetxt(f"../modules/fsh_temp/{file_name}",datas[thermalization_range[0]:thermalization_range[1]],fmt='%.18f')
...
def compute_with_fsh_jackknife(data, column, bins, system_size, range_and_acc_list, path="/home/haaaaron/SUN_twist_python_analysis/modules/fsh_temp/", thermalization_range=(1000,2000)):
    all_jackknife_data = []
    fsh_list_lines = []
    beta_values = []
    twist_coeff = None
